...
def main():
	...
	if rank == 0:
		...
		slave_tweet_processor(comm, sal_dict, data_url)
...
def master_tweet_processor(comm, sal_dict, data_url) -> None:
	...
	if size > 1:
		res = summerise_data(comm, task_1, task_2, task_3)
		task_1 = res['task_1']
		task_2 = res['task_2']
		task_3 = res['task_3']
...
def slave_tweet_processor(comm, sal_dict, data_url):
	rank = comm.Get_rank()
	size = comm.Get_size()
	gcc_count = {'1gsyd': 0, '2gmel': 0, '3gbri': 0, '4gade': 0, '5gper': 0,'6ghob': 0, '7gdar': 0, '8acte': 0, '9oter': 0}
	user_record = {}
...
def summerise_data(comm, master_gcc_count, master_tweet_count, master_unique_gcc_count) -> ({}, {}, {}):
